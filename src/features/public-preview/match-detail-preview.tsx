...
import { PUBLIC_PREVIEW_TOURNAMENT } from "./fixtures";
import { formatPreviewDateTime, formatPreviewSyncTime, formatPreviewTime } from "./format";
import { correctionLabels, outcomeLabels, type PreviewScenario, type PublicMatchPreview } from "./model";
...
export interface MatchDetailPreviewProps {
  match: PublicMatchPreview;
  preservedQuery: string;
  scenario: PreviewScenario;
}

export function MatchDetailPreview({ match, preservedQuery, scenario }: MatchDetailPreviewProps) {
  const scheduleHref = `/public/preview/${PUBLIC_PREVIEW_TOURNAMENT.slug}/schedule${preservedQuery ? `?${preservedQuery}` : ""}`;
  const retryQuery = new URLSearchParams(preservedQuery);
  retryQuery.delete("scenario");
  const retryHref = `/public/preview/${PUBLIC_PREVIEW_TOURNAMENT.slug}/matches/${match.code}${retryQuery.size ? `?${retryQuery}` : ""}`;
  const sideAName = match.sideA?.displayName ?? "对阵待定";
  const sideBName = match.sideB?.displayName ?? "对阵待定";
  const time = formatPreviewTime(match.time);

  return (
    <PublicResultsShell activeSection="match" tournament={PUBLIC_PREVIEW_TOURNAMENT}>
      <div className={styles.detailToolbar}>
        <Link className={styles.backLink} href={scheduleHref}>← 返回每日赛程</Link>
        {scenario === "ready" || scenario === "stale" ? <span>最后同步 {formatPreviewSyncTime(match.lastSyncedAt)}</span> : null}
      </div>

      {scenario === "loading" ? <PublicMatchSkeleton /> : null}
      {scenario === "error" ? (
        <ResourceState
          action={<Link className={styles.primaryLink} href={retryHref}>重新读取模拟详情</Link>}
            ...
            <aside aria-label="比赛状态与信息" className={styles.detailAside}>
              ...
              <section>
                ...
                <dl className={styles.detailFacts}>
                  ...
                  <div><dt>实际开始</dt><dd>{formatPreviewDateTime(match.startedAt)}</dd></div>
                  <div><dt>实际结束</dt><dd>{formatPreviewDateTime(match.endedAt)}</dd></div>
                  <div><dt>比分版本</dt><dd>{match.scoreVersion}</dd></div>
                  <div><dt>公开投影版本</dt><dd>{match.projectionRevision}</dd></div>
                </dl>
              </section>
              <section>
                <h2>规则摘要</h2>
