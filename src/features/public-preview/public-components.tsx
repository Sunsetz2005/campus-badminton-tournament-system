...
export function PublicResultsShell({ activeSection, children, tournament }: PublicResultsShellProps) {
  return (
    <section
      className={`${styles.shell} ${activeSection === "schedule" ? styles.scheduleShell : ""}`}
      data-public-preview="true"
    >
      ...
      <header className={styles.eventHeader}>
        ...
        <dl className={styles.eventFacts}>
          ...
          <div><dt>场馆</dt><dd>{tournament.venue}</dd></div>
          <div><dt>赛事时区</dt><dd>{tournament.timezone}</dd></div>
        </dl>
      </header>
      <p className={styles.announcement}>{tournament.announcement}</p>
      {children}
    </section>
  );
}
...
export function EntryIdentity({ entry, side }: EntryIdentityProps) {
  ...
  return (
    <div className={styles.entryIdentity}>
      <span>{side} 方 · {entry.teamName}</span>
...
export function ScheduleMatch({ detailHref, match, presentation }: ScheduleMatchProps) {
