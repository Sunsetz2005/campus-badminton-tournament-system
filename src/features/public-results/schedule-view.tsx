...
import {
  filterPreviewMatches,
  parseScheduleQuery,
  preserveScheduleParams,
  withScheduleQuery,
  type ScheduleQueryOptions,
} from "./query";

export interface PublicScheduleViewProps {
  /** 比赛详情地址前缀，例如 `/public/autumn-campus-2026/matches`。 */
  matchHrefBase: string;
  /** 赛事已发布日期中的默认日期；由服务端给出，不按设备时区推导。 */
  defaultDate: string;
  /** 该赛事全部已发布且有赛程日期的比赛，已按服务端权威顺序排好。 */
  matches: readonly PublicMatchPreview[];
  /**
   * `preview` 是隔离模拟夹具，显示「模拟数据 · 界面预览」横幅并开放预览状态选择器；
   * `live` 读取真实公开投影，不提供伪造加载/失败/过期状态的开关。
   */
  mode: "preview" | "live";
  queryOptions: ScheduleQueryOptions;
  tournament: PublicTournamentPreview;
  /** 已发布但尚无赛程日期的比赛；不硬塞进任何一天。 */
  unscheduled?: readonly PublicMatchPreview[];
}
...
export function PublicScheduleView({
  ...
  const parsed = useMemo(
    ...
    [defaultDate, queryOptions, searchParams],
  );
  ...
  const competitionOptions = useMemo(
    () => [...new Map(matches.map((match) => [match.competitionCode, match.competitionName])).entries()],
    [matches],
  );
  const stageOptions = useMemo(
    () => [...new Map(matches.map((match) => [match.stageCode, match.stageName])).entries()],
    [matches],
  );
  const courtOptions = useMemo(
    () => [...new Set(matches.map((match) => match.court).filter((court): court is string => Boolean(court)))],
    [matches],
  );
  ...
  // 预览状态只属于隔离夹具。真实公开页不提供伪造加载/失败/过期的开关。
  const scenario: PreviewScenario = mode === "preview" ? query.scenario : "ready";
  const invalidDate = parsed.issues.some((issue) => issue.key === "date" && issue.reason === "INVALID_DATE");
  const forcedEmpty = scenario === "empty";
  const dayMatches = invalidDate || forcedEmpty
    ? []
    : matches.filter((match) => match.scheduleDate === query.date);
  const matching = invalidDate || forcedEmpty ? [] : filterPreviewMatches(matches, query);
  ...
  const showResolvedData = !invalidDate && scenario !== "loading" && scenario !== "error";
  ...
  function detailHref(code: string) {
    ...
    return suffix ? `${matchHrefBase}/${code}?${suffix}` : `${matchHrefBase}/${code}`;
  }

  return (
    <PublicResultsShell activeSection="schedule" mode={mode} tournament={tournament}>
      ...
      {showResolvedData && !forcedEmpty ? (
        <div className={styles.resultsToolbar}>
          ...
          <div>
            {scenario === "stale" ? <span className={styles.staleNotice} role="status">连接中断 · 数据可能过期</span> : null}
      ...
      {scenario === "stale" ? (
        <p className={styles.syncNotice} role="status">保留最后一次成功数据；最后同步 {formatPreviewSyncTime(tournament.updatedAt, tournament.timezone)}，不会清空或伪造比分。</p>
      ) : null}

      {scenario === "loading" ? <PublicScheduleSkeleton /> : null}
      {!invalidDate && scenario === "error" ? <PublicErrorState retryHref={withScheduleQuery(pathname, currentParams, { scenario: null }, queryOptions)} /> : null}
      {invalidDate ? (
          ...
          <div className={styles.scheduleTableWrap}>
            <table className={styles.scheduleTable}>
              ...
              <tbody>{matching.map((match) => (
                <ScheduleMatch
                  detailHref={detailHref(match.code)}
                  key={match.code}
                  match={match}
      ...
      {showResolvedData && unscheduled.length ? (
        <section aria-labelledby="unscheduled-heading" className={styles.unscheduled}>
          <h2 id="unscheduled-heading">待排期（{unscheduled.length} 场）</h2>
          <p>以下比赛已公开但尚未确定日期，不归入任何一天。</p>
          <div className={styles.scheduleCards}>
