...
- 阶段 0、阶段 1 和阶段 2 已完成；S2-008 的换边待办关系已收口，S2-009 已实现并通过本地回归，两者均待独立复验。
- 阶段 3 的服务端计分事务、租约心跳/接管、响应未知恢复和场地图裁判工作台已通过本地自动化验收。场地图只投影权威状态，支持单双打、拟首发/拟首接、双打手动换位、物理换边、本机翻转视角、减分预览和 `UNKNOWN` 原 `commandId` 对账。
- 本轮证据为定向测试 64/64、`pnpm test` 94/94、Playwright 13/13、lint/typecheck/build 通过、生产启动安全 3/3、知识库同步脚本测试 10/10，以及 375/390/1024 视口和 200% 字体视觉 QA 通过。正式 BWF 规则采用、真机/真人裁判、生产部署和独立 Claude 复验仍未完成。
